...
(defun org-import-icalendar-get-deadline-and-schedule (ical-element)
  ""
  (let* ((dtstart (icalendar--get-event-property e 'DTSTART))
    ...
    (setq end-t (if (and
                  ...
                  start-t))
    (concat "DEADLINE: " (format "<%s %s>" end-d end-t) " SCHEDULED: " (format "<%s %s>" start-d start-t))))
...
(defun org-import-icalendar-import-element (ical-element)
  (let* ((location (chomp (icalendar--get-event-property e 'LOCATION)))
         (organizer (icalendar--get-event-property e 'ORGANIZER))
         (uid (icalendar--get-event-property e 'UID))
         (raw-rrule (chomp (icalendar--get-event-property e 'RRULE)))
         (description (chomp (icalendar--get-event-property e 'DESCRIPTION)))
         ...
         ;;
         (location-str (if (string= "" location) "" (concat "\n:LOCATION: " location)))
         (rrule-str (if raw-rrule (concat "\n:RRULE: " raw-rrule) ""))
         (rrule (let ((match t)
                      (start 0)
                      (keyvalues (list)))
                  (while match
                    (let ((found (string-match "\\([^=;]+\\)=\\([^=;]+\\)" raw-rrule start))
                          (key (match-string 1 raw-rrule))
                          (value (match-string 2 raw-rrule))
                          (end (match-end 0)))                          
                      (setq match found)
                      (setq start end)
                      (when key
                        (setq keyvalues (cons (list key value) keyvalues)))
                      ))
                  keyvalues))
         (msg (format "** TODO %s\n%s\n:PROPERTIES:\n:UID: %s%s%s\n:END:\n%s\n\n" summary (org-import-icalendar-get-deadline-and-schedule ical-element)  uid location-str rrule-str  description )))

    (message "Added to kill-ring:\n %s" msg)
    (if (not org-import-icalendar-filename)
        ;; Store in kill-ring
        (prog (message "Do NOT append to org-import-icalendar-filename") (kill-new msg))
      ;; else: append to file
      (with-temp-file org-import-icalendar-filename
        (message "Append to org-import-icalendar-filename")
